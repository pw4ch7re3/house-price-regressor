...
def get_latlong_cols(df):
    ...
    if lat_col is None or lon_col is None:
        raise ValueError(
            f"lat/long columns not found. Available columns: {df.columns.tolist()}"
        )
...
def clean_numeric(X_train, X_test):
    ...
    if len(obj_cols) > 0:
        X_train_ohe = pd.get_dummies(
            X_train[obj_cols].astype(str), prefix=obj_cols, dtype=float
        )
        X_test_ohe = pd.get_dummies(
            X_test[obj_cols].astype(str), prefix=obj_cols, dtype=float
        )
...
def train(
    ...
    torch.save(
        model.state_dict(), os.path.join(save_dir, f"best_{model_name}_{target}.pth")
    )
...
def drop_target_outliers(df, target_col, upper_quantile):
    """
    target 기준 outlier 제거.
    - target <= 0 인 invalid row 제거
    - target 이 upper_quantile 분위수를 넘는 극단값 제거
    split 이전, 전체 df 에 대해 적용한다 (train/test 모두 같은 기준).
    """
    s = df[target_col]
    ...
    print(
        f"drop_outliers: target={target_col} "
        f"upper_quantile={upper_quantile} cap={cap:.2f} "
        f"removed={n_before - n_after} ({n_before} -> {n_after})"
    )
...
def run_one_experiment(args, target_col):
    ...
    # outlier ablation: --drop_outliers 켰을 때만 제거
    if args.drop_outliers:
        df = drop_target_outliers(df, target_col, args.outlier_quantile)
    ...
    if args.model == "mlp":
        X_train, X_test = preprocess_for_mlp(
            X_train, y_train, X_test, args.location_case
        )
        ...
        X_train, X_test = preprocess_for_dt(
            X_train, y_train, X_test, args.location_case
        )
    ...
    print(f"drop_outliers: {args.drop_outliers} (quantile={args.outlier_quantile})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Location case experiment with normalized target RMSE"
    )
