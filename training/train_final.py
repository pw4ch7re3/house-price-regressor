...
# Stacked target-propagation xgb layers (matches the mgbdt path in train.py).
LAYER_CONFIGS = [("tp_layer", "xgb", 8), ("tp_layer", "xgb")]


def build_model_config(args, input_size: int) -> mGBDTConfig:
    return mGBDTConfig(
        model="mgbdt",
...
def train(model_config: ModelConfig, train_config: TrainConfig, target_name: str):
    ...
    torch.save(
        model.state_dict(),
        os.path.join(output_path, f"final_mgbdt_{target_name}.pth"),
    )
...
def train_and_predict(
    X_tr, y_tr, X_ev, y_ev, drop_address, args, base_train_kwargs, save_models
):
    ...
    # mGBDT needs the (post-drop) feature count up front.
    model_config = build_model_config(args, X_tr.shape[1])
...
def cross_validate(
    X_train, y_train, drop_address, args, base_train_kwargs, k_folds, seed
):
    ...
    for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train), start=1):
        ...
        preds, w, n_features = train_and_predict(
            X_tr, y_tr, X_val, y_val, drop_address, args, base_train_kwargs,
            save_models=False,
        )
        weights.append(w)
...
def report_cv(fold_metrics, weights, drop_address, k_folds):
    ...
    for target in TARGETS:
        ...
        save_split_metrics(target, "ensemble_mgbdt", drop_address, True, "cv", means)
...
def main(args):
    ...
    fold_metrics, weights = cross_validate(
        X_train, y_train, args.drop_address, args, base_train_kwargs,
    ...
    preds, best_w, n_features = train_and_predict(
        X_train, y_train, X_test, y_test, args.drop_address, args,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train an mGBDT ensemble: blend a direct price model with a "
